...
from .models import Product as Item
from .schemas import product_schema as schema
from .schemas import product_schema_many as schema_many
...
def get_all_items():
    ...
    return get_all_items_from_page(1, 'shop')


def get_all_items_from_page(page_num, product_filter='shop'):
    """Deserialize and return all items in database"""
    
    total_item = Item.objects(category=product_filter).count()
    ...
    items = schema_many.dump(Item.objects(category=product_filter).skip(skip_item).limit(item_per_page))
    return {
        'items': items, 
