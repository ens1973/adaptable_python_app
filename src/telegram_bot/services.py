...
from .models import TelegramBot as Item
from .schemas import telegrambot_schema as schema
from .schemas import telegrambot_schema_many as schema_many
...
def get_item_by_name(item_name):
    """Given an item ID, return a serialized item object"""
    if not (item := Item.objects(name__exact=item_name).first()):
        return {"message": does_not_exist(item_name)}, 404
